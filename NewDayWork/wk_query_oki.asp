...
查詢條件：工作主旨=[<font color=red><%=q_text%></font>]及工作人員=[<font color=red><%=wk_man_t%></font>]
</font>
<hr>
<% if worker="木偉" then %>
<form name="form1" action="wk_edit_item_ok.asp" method="post">
整體修改工作主旨：
原完整主旨：<input type='text' name='wk_item_old' value='<%=q_text%>' style="width:200px;" >
新完整主旨：<input type='text' name='wk_item_new' value='<%=wk_item%>' style="width:200px;" >
<input type=submit name="editok" value="確定修改" style="width:100px;">
</form>
<% end if %>
<hr>
<%
...
strSQL_show="Select * from " & tb_name & " where wk_group like '%"&wkgroup&"%' and wk_item like '%"&q_text&"%' and wk_doer like '%"& wk_man &"%' order by doing_date1 desc"
