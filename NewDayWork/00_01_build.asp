...
'strSQL_acr="Select * from " & tb_name_acr &" where (wk_gp like '總經理室' or wk_gp like '企劃部' or wk_gp like '資訊部' or wk_gp like '建設部' ) and not(e_name like 'Larry') and not((hide=true) and (st_qdate <= #"& date() &"# )) order by wk_sgp asc, wk_gp_sq, wkr_id asc"
'strSQL_acr="Select * from " & tb_name_acr &" where ((e_name like 'Eagle' or e_name like 'Allan' or e_name like 'Danny') or wk_gp like '企劃部' or wk_gp like '資訊部' or wk_gp like '建設部' ) and not(e_name like 'Larry') and not((hide=true) and (st_qdate <= #"& date() &"# )) order by wk_sgp asc, wk_gp_sq, wkr_id asc"
strSQL_acr="Select * from " & tb_name_acr &" where ((e_name like 'Eagle' or e_name like 'Allan' or e_name like 'Danny') or wk_gp like '%企劃部%' or wk_gp like '%資訊部%' or wk_gp like '%建設部%' ) and not(e_name like 'Larry') and not((hide=true) and (st_qdate <= #"& date() &"# )) order by wk_sgp asc, wk_gp_sq, wkr_id asc"
...
'將frm_main的網頁轉址到切換使用者登入前之畫面
         str_url="http://60.251.159.62:6980/build/daywork/firstpage.asp?paswd=28283939"
...
<body>
<center>
<form name="form_login" method=post action="00_01_build.asp">

<font style="font-family:'標楷體';font-size:30px;font-weight:bold;letter-spacing:15px;">喬大建設部管理系統</font>
<hr width=300>
<font style="font-size:20px;font-weight:bold;letter-spacing:15px;">【登入畫面】</font>
<br>
<hr color=red> 
<table border=0 cellspacing=0 cellpadding=2 style="width:500px" >
