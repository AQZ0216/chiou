...
strSQL_acr="Select * from " & tb_name_acr &" where ((e_name like 'Eagle' or e_name like 'Allan' or e_name like 'Danny') or wk_gp like '%企劃部%' or wk_gp like '%資訊部%' or wk_gp like '%法務部%' ) and not(e_name like 'Larry') and not((hide=true) and (st_qdate < #"& date() &"# )) order by wk_sgp asc, wk_gp_sq, wkr_id asc"
'strSQL_acr="Select * from " & tb_name_acr &" where ((e_name like 'Eagle' or e_name like 'Allan' or e_name like 'Danny') or wk_gp like '企劃部' or wk_gp like '資訊部' or wk_gp like '法務部' ) and not(e_name like 'Larry') and not((hide=true) and (st_qdate < #"& date() &"# )) order by wk_sgp asc, wk_gp_sq, wkr_id asc"
...
'將frm_main的網頁轉址到切換使用者登入前之畫面
         str_url="./00_05_law_page.asp?p_uid="& p_userid
...
<body>
<center>
<form name="form_login" method=post action="00_05_law.asp">

<font style="font-family:'標楷體';font-size:30px;font-weight:bold;letter-spacing:15px;">喬大法務部管理系統</font>
<hr width=300>
...
<hr color=red>
	<font onclick="parent.location.href='../zone00/00_ts_main.asp'" style="cursor:hand;font-size:20px;font-weight:bold;letter-spacing:15px;background-color:#c3c3c3;">　謄本系統(新版)　</font>
<hr color=red>
