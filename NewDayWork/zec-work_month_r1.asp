...
if cint(p_month)=13 then
   p_year=p_year+1
   p_month=1
elseif p_month=0 then
   p_year=p_year-1
   p_month=12
end if
...
%>
      ...
      <table class="w3-table-all" ><!-- 功能表 start -->
         ...
         <tr style="border:1px solid #000;height:55px;">
           <td style="text-align:center;">
            ...
            <button class="w3-button w3-white w3-xlarge " style="padding:2px;margin:0px;" >【<%=p_year%>年<%=p_month%>月】單月工作項目</button>
...
%>          
      <td style="background-color:<%=div_background_c%>;border-color:<%=div_border_c%>;" >
         <span class="w3-button w3-black" title="顯示今日工作" style="padding:0px;margin:0px;" onclick="url_show('zec-work_day_r1.asp?datecode=<%=pndate%>&worker=<%=worker%>')" ><%=pndate%>(<%=str_wk%>)</span>
<!--         <button class="w3-button w3-grey w3-medium " style="padding:0px;margin:0px;">【增】</button>-->
         <span class="w3-button w3-red" title="新增工作" style="padding:0px;margin:0px;" onclick="url_show('zec-work_add_r1.asp?datecode=<%=pndate%>&worker=<%=worker%>')" > ＋ </span>
         <br>
