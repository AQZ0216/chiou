...
wkgroup="一般工作"
	...
	'讀取今天日期
	wk_code=request("wk_code")
select case wk_code
	Case "完成工作"
strSQL_show="Select * from " & tb_name & " where wk_group like '%"&wkgroup&"%' and wk_finisher like '%"& wk_man &"%' order by doing_date1 desc"
	Case "執行工作"
strSQL_show="Select * from " & tb_name & " where wk_group like '%"&wkgroup&"%' and wk_undoer like '%"& wk_man &"%' order by doing_date1 desc"
	Case "預計工作"
...
查詢條件：工作分類=[<font color=red><%=wk_code%></font>]及工作人員=[<font color=red><%=wk_man_t%></font>] 
</font>
<%
'建立資料庫存取物件	
